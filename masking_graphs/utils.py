...
def split_solution_keep_spacing(
    solution_text: str,
    get_idxs: bool = False,
    drop_think: bool = True,
    linebreak_before: bool = False,
) -> List[str]:
    """
    Split a solution into chunks while preserving spacing.
    """
    # Define patterns for chunk boundaries
    sentences = split_solution_into_chunks(solution_text, drop_think)

    chunk_ranges = get_chunk_ranges(solution_text, sentences)
    chunk_ranges = [list(chunk_range) for chunk_range in chunk_ranges]
    sentences_w_spacing = [
        solution_text[chunk_range[0] : chunk_range[1]] for chunk_range in chunk_ranges
    ]

    if linebreak_before:
        sentences = sentences_w_spacing
        sentences_mod = []
        for i in range(len(sentences) - 1):
            assert chunk_ranges[i][1] == chunk_ranges[i + 1][0]
            if sentences[i][-2:] == "\n\n":
                sentences[i] = sentences[i][:-2]
                sentences[i + 1] = "\n\n" + sentences[i + 1]
                sentences_mod.append(sentences[i])
                chunk_ranges[i][0] = chunk_ranges[i][0] - 2
                # chunk_ranges[i + 1][0] = chunk_ranges[i + 1][0] + 2
            elif sentences[i][-1] == "\n":
                sentences[i] = sentences[i][:-1]
                sentences[i + 1] = "\n" + sentences[i + 1]
                sentences_mod.append(sentences[i])
                chunk_ranges[i][0] = chunk_ranges[i][0] - 1
                # chunk_ranges[i][1] = chunk_ranges[i][1] - 1
                # chunk_ranges[i + 1][0] = chunk_ranges[i + 1][0] + 1
            else:
                sentences_mod.append(sentences[i])
        sentences_mod.append(sentences[-1])
        sentences_w_spacing = sentences_mod
    chunk_ranges = get_chunk_ranges(solution_text, sentences_w_spacing)

    if get_idxs:
        return sentences_w_spacing, [chunk_range[0] for chunk_range in chunk_ranges]
...
def diag_multi_k(arr, k_start=5, k_end=10, pad_value=np.nan):
    """
    Extract multiple diagonals and return as a 2D array (with padding if needed).
    ...
    arr : numpy.ndarray
        2D input array
    k_start : int
        Starting diagonal offset (default: 5)
    k_end : int
        Ending diagonal offset (inclusive) (default: 10)
    pad_value : scalar
        Value to use for padding shorter diagonals (default: np.nan)

    Returns:
    --------
    numpy.ndarray : 2D array where each row is a diagonal (padded to same length)

    Example:
    --------
    >>> matrix = np.arange(144).reshape(12, 12)
    >>> diag_array = get_diagonals_as_array(matrix, 5, 10)
    ...
    # Extract diagonals and find maximum length
    for k in range(k_start, k_end + 1):
        diag = np.diag(arr, k=k)
        diagonals.append(diag)
        max_len = max(max_len, len(diag))

    # Pad diagonals to same length
    padded_diagonals = []
    for diag in diagonals:
        if len(diag) < max_len:
            padded = np.pad(diag, (0, max_len - len(diag)), constant_values=pad_value)
        else:
            padded = diag
        padded_diagonals.append(padded)

    return np.array(padded_diagonals)
