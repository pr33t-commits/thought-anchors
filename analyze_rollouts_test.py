...
from chunking_mod import resolve_chunk_artifact_paths, resolve_chunk_dir
import re
...
def calculate_resampling_importance_kl(
    ...
    _, _, use_mod_chunks = resolve_chunk_artifact_paths(problem_dir)
    chunk_dir = resolve_chunk_dir(problem_dir, chunk_idx, use_mod_chunks)

    # Get next chunk
    next_chunk_idx = min(next_chunks)
    next_chunk_dir = resolve_chunk_dir(
        problem_dir, next_chunk_idx, use_mod_chunks
    )
...
def calculate_forced_importance_kl(
    ...
    if not forced_problem_dir.exists():
        return 0.0
    _, _, forced_use_mod_chunks = resolve_chunk_artifact_paths(
        forced_problem_dir
    )

    # Get current chunk answers
    current_chunk_dir = resolve_chunk_dir(
        forced_problem_dir, chunk_idx, forced_use_mod_chunks
    )
    current_solutions = []
    ...
    next_chunk_dir = resolve_chunk_dir(
        forced_problem_dir, next_chunk_idx, forced_use_mod_chunks
    )
    next_solutions = []
...
def analyze_problem(
    ...
    chunks_file, labeled_chunks_file, use_mod_chunks = (
        resolve_chunk_artifact_paths(problem_dir)
    )
    ...
    chunk_folders = [
        resolve_chunk_dir(problem_dir, i, use_mod_chunks)
        for i in valid_chunk_indices
    ]
    ...
    if force_relabel:
        print(
            f"Problem {problem_dir.name}: force_relabel ignored because labeling now happens before analysis"
        )

    if not labeled_chunks_file.exists():
        print(
            f"Problem {problem_dir.name}: Missing precomputed labels at {labeled_chunks_file}"
        )
        return None
...
def process_rollouts(
    ...
    for problem_dir in problem_dirs:
        chunks_file, _, use_mod_chunks = resolve_chunk_artifact_paths(
            problem_dir
        )
